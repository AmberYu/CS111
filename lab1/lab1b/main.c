...
int main(int argc, char * argv[])
{
    ...
    int fd_vec[argc];
    //not pipe 0, readend 1, writeend 2
    int is_pipe[argc];
            ...
            case COMMAND:
            {
                ...
                if (pid == 0) {
                    ...
                    if(optarg){
                        ...
                        //if is readend close the writeend
                        if(is_pipe[atoi(argv[optind])] == 1)
                            close(fd_vec[atoi(argv[optind+1])]);
                        
                        //if is write end close the read end
                        if(is_pipe[atoi(argv[optind])] == 2)
                            close(fd_vec[atoi(argv[optind-1])]);
