		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		osp_spin_unlock(&(d->mutex));
		// eprintk("d->notification_ticket_tail == my_ticket is %d\n", d->notification_ticket_tail == my_ticket);
		// eprintk("is_notified is %d\n", is_notified(d->wait_notification_pids, current->pid));
 		int tmp = wait_event_interruptible(d->block_notify,d->notification_ticket_tail == my_ticket
										&& is_notified(d->wait_notification_pids, current->pid));
		if(tmp){
			osp_spin_lock(&(d->mutex));
